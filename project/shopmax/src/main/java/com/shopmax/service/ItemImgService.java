    ...
    */
    ...
    public void saveItemImg(ItemImg itemImg, MultipartFile itemImgFile) throws Exception {
        ...
        if (!StringUtils.isEmpty(oriImgName)) { //빈문자열인지 아닌지 검사
            ...
            //itemImgFile.getBytes(): 이미지 파일을 byte 배열로 만들어준다.(스트링에서 사용하기 위해)

            imgUrl = "/images/item/" + imgName; // /images/item/EERERERT-ERERERER-CDFDGD.jpg
        }

        //DB에 insert를 하기 전 유저가 직접 입력하지 못하는 값들은 개발자가 넣어준다.
    ...
    public void updateItemImg(Long itemImgId, MultipartFile itemImgFile) throws Exception {
        if (!itemImgFile.isEmpty()) { //첨부한 이미지 파일이 있으면
            ...
            String oriImgName = itemImgFile.getOriginalFilename(); //테스트
            String imgName = fileService.uploadFile(itemImgLocation, oriImgName,
                    itemImgFile.getBytes());
            String imgUrl = "/images/item/" + imgName; // /images/item/-.jpg
